...
var supportedOptions = ['prettifyUrls', 'sources', 'recursiveSources', 'maxRecursiveDepth', 'defaultFilename', 'destinationUrl', 'relativePathFilter'];
	...
	var childrenPromises = childrenPaths.map(function loadChildPath (childPath) {
		...
		return self.context.requestResource(childResource).then(function updateChildPath (respondedResource) {
			if (respondedResource) {
				...
				relativePath = self.context.options.relativePathFilter(relativePath, self.context.options.destinationUrl);
